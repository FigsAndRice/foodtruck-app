...
export default class Countdown extends Component {
  ...
  render() {
    if (this.props.hours > Date.now()) {
      return (
        <Text style={style.timer}>{`${String('0' + this.state.hoursLeft).slice(-2)}:${String('0' + this.state.minutesLeft).slice(-2)}:${String('0' + this.state.secondsLeft).slice(-2)} left.`}</Text>
      )
    } else {
